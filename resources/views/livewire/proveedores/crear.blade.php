<dialog id="my_modal_2" class="modal">
    <div class="modal-box bg-white dark:bg-gray-800 text-gray-900 dark:text-white rounded-md min-w-3/5">
        <h3 class="text-lg font-bold mb-6">Crear Nuevo Proveedor</h3>
        <form wire:submit.prevent="crear()" id="form-crear-proveedor" class="grid gap-6 md:grid-cols-2 p-4">
            ...
            <div>
                <label for="countries"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tipo Ajudicación</label>
                <select id="countries" wire:model="tipo_adjudicacion_id"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    required>
                    <option selected disabled>Seleccione</option>
                    @foreach ($tipo_adjudicaciones as $item)
                        <option value="{{ $item->id }}">{{ $item->nombre_tipo_adjudicacion }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <label for="countries"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tipo Proveedor</label>
                <select id="countries" wire:model="tipo_proveedor_id"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    required>
                    <option selected disabled>Seleccione</option>
                    @foreach ($tipo_proveedores as $item)
                        <option value="{{ $item->id }}">{{ $item->nombre_tipo_proveedor }}</option>
                    @endforeach
                </select>
        ...
        <div class="modal-action">
            <form>
                ...
                <button type="submit" form="form-crear-proveedor"
