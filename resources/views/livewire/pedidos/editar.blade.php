<div>
        ...
        <div class="flex flex-column sm:flex-row flex-wrap space-y-1 sm:space-y-0 items-center justify-between ">
            <div class="items-center gap-8">
                <div class="text-2xl font-semibold text-gray-900 dark:text-white">
                    Editar Lista de Productos del Pedido
                </div>
    ...
    <div action="">
                                        ...
                                        <div class="flex items-center">
                                            <div>
                                                <input type="number" id="first_product-{{ $item->id }}"
                                                    wire:model.live="productoCantidad.{{ $item->id }}"
                                                    ...
                                                    placeholder="Ingrese" required min="0" autocomplete="off" />
                                            </div>

                                        </div>
                                    </td>
                                    <td class="px-6 py-1">
                                        <button @if (empty($productoCantidad[$item->id]) || $productoCantidad[$item->id] < 1) disabled @endif
                                            wire:click="agregarProducto({{ $item->id }})" type="button"
                        ...
                        <tbody>
                            @foreach ($resumenPedido as $item)
                                <tr
                                    ...
                                    <td class="px-6 py-1">
                                  <button wire:click="quitarProducto({{ $item['producto_id'] }})" type="button" class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-1.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">Quitar</button>
                                </td>
                                </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="pl-6 pr-6 mb-6">
            ...
            <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Observación (Opcional)</label>
        ...
        <div class="w-full flex justify-between">
            <x-back-button ruta="pedidos.detalles" param="{{ $pedido->id }}" label="Regresar" />
            @if ($resumenPedido)
            @can('Editar Pedidos')
