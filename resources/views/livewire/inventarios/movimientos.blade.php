 ...
 <div id="modal-editar-detalle" tabindex="-1" aria-hidden="true"
     ...
     <div class="relative p-4 w-full h-full flex justify-center items-center">
         ...
         <!-- Modal content -->
         <form wire:submit.prevent="movimientosGuardar()" class="relative bg-white rounded-lg shadow-sm dark:bg-gray-800 w-2/5">
             <!-- Modal body -->
             <div class="mb-3 p-5">
                 ...
                 <h3 class="text-base mb-6 text-md text-gray-900 dark:text-white">Cant. disponible: {{ $cantidad_disponible }}</h3>
                 ...
                 @if ($tipo_movimiento)
                     ...
                     <input type="number" id="first_name" wire:model.live="cantidad"
                         class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                         placeholder="" required max="{{ $cantidad_disponible }}" autocomplete="off" required />
                     @error('cantidad')
                         <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
                     @enderror
                 </div>
